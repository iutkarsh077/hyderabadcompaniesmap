...
import { getPuneListings } from "@/lib/pune";
import type { Company } from "@/types/company";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Pune directory",
  description:
    "Text directory of startups and companies in Pune, grouped by industry, with short descriptions and websites.",
};
...
export default async function PuneDirectoryPage() {
  let companies: Company[] = [];
  let error: string | null = null;

  try {
    companies = await getPuneListings();
  } catch (caught) {
  ...
  return (
    <article>
      <h1 className="text-3xl font-semibold tracking-tight">Pune directory</h1>
      <p className="mt-3 max-w-3xl text-[15px] leading-relaxed text-[#344054]">
        ...
        . Each entry is a company with a known or approximate location in Pune. Coordinates on the
        map are district-level, not exact street addresses. These listings are not shown on the
        Hyderabad, Bengaluru, or Delhi NCR maps. This site is independent of{" "}
        <a
          className="font-medium text-[#3d7a2f]"
          href="https://www.punestartupmap.in/"
          target="_blank"
          rel="noreferrer"
        >
          Pune Startup Map
      ...
      {error ? (
        ...
        <p className="mt-6 text-sm text-[#667085]">
          No Pune listings in the database yet. Load a snapshot with{" "}
          <code className="text-[13px]">npm run import:pune</code>.
        </p>
      ) : (
        <>
