...
import { getAhmedabadListings } from "@/lib/ahmedabad";
import type { Company } from "@/types/company";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Ahmedabad directory",
  description:
    "Text directory of startups and companies in Ahmedabad and GIFT City, grouped by industry, with short descriptions and websites.",
};
...
export default async function AhmedabadDirectoryPage() {
  let companies: Company[] = [];
  let error: string | null = null;

  try {
    companies = await getAhmedabadListings();
  } catch (caught) {
  ...
  return (
    <article>
      <h1 className="text-3xl font-semibold tracking-tight">Ahmedabad directory</h1>
      <p className="mt-3 max-w-3xl text-[15px] leading-relaxed text-[#344054]">
        ...
        . Each entry is a company with a known or approximate location in Ahmedabad or GIFT City.
        Coordinates on the map are district-level, not exact street addresses. These listings are
        not shown on the Hyderabad, Bengaluru, Delhi NCR, or Pune maps. This site is independent of{" "}
        <a
          className="font-medium text-[#3d7a2f]"
          href="https://ahmedabad-jobs-map.vercel.app/"
          target="_blank"
          rel="noreferrer"
        >
          Ahmedabad Jobs Map
      ...
      {error ? (
        ...
        <p className="mt-6 text-sm text-[#667085]">
          No Ahmedabad listings in the database yet. Load a snapshot with{" "}
          <code className="text-[13px]">npm run import:ahmedabad</code>.
        </p>
      ) : (
        <>
